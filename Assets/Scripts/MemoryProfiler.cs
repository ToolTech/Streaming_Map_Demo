...
public class MemoryProfiler : MonoBehaviour, IProfiler
{
    ...
    public TMP_Text Text => TMpro;

    public void StartBenchmark()
    {
        if (IsRunning)
        {
            Debug.LogWarning("MemoryBencmark is already running");
            return;
        }
    ...
    public void StopBenchmark()
    {
        if (!IsRunning)
        {
            Debug.LogWarning("MemoryBencmark is not running");
            return;
        }
        IsRunning = false;
