				...
				void geom(point uint p[1] : TEXCOORD, inout TriangleStream<FramentInput> triStream)
				{
					...
					if (GemerateGeometry(p[0], grassPosition, displacement, displacementx, size, tilt, tiltx, bottomColor, topColor, uvDistortion, textureWaving, index))
					{
						...
						if (tiltx <= 1 && tiltx >= -1)
						{
							...
							// Top vertices
							AppendVertex(triStream, grassPosition, displacement + half4(0, size.y, 0, 0), upNormal, fixed3(0 + textureWaving.x, 0.5f, index), topColor);
							AppendVertex(triStream, grassPosition, -displacement + half4(0, size.y, 0, 0), upNormal, fixed3(0.5 + textureWaving.y, 0.5f, index), topColor);

							// Bottom vertices
							AppendVertex(triStream, grassPosition, displacement, upNormal, fixed3(0, 0.0f, index), bottomColor);
							AppendVertex(triStream, grassPosition, -displacement, upNormal, fixed3(0.5f, 0.0f, index), bottomColor);
						}
						...
						if (tilt <= 1 && tilt >= -1)
						{
							...
							AppendVertex(triStream, grassPosition, displacementx + half4(0, size.y, 0, 0), upNormal, fixed3(0 + textureWaving.x, 1, index), topColor);
							AppendVertex(triStream, grassPosition, -displacementx + half4(0, size.y, 0, 0), upNormal, fixed3(0.5 + textureWaving.y, 1, index), topColor);

							// Bottom vertices (crossed)
							AppendVertex(triStream, grassPosition, displacementx, upNormal, fixed3(0 , 0.5f, index), bottomColor);
							AppendVertex(triStream, grassPosition, -displacementx, upNormal, fixed3(0.5 , 0.5f, index), bottomColor);
						}
						...
						if (yaw < 0)
						{
							...
							// Top vertices (crossed)
							AppendVertex(triStream, grassPosition, displacementx + half4(0, size.y / 2, 0, 0) + displacement, upNormal, fixed3(0.5, 0.5f, index), topColor);
							AppendVertex(triStream, grassPosition, -displacementx + half4(0, size.y / 2, 0, 0) + displacement, upNormal, fixed3(1, 0.5f, index), topColor);

							// Bottom vertices (crossed)
							AppendVertex(triStream, grassPosition, displacementx + half4(0, size.y / 2, 0, 0) - displacement, upNormal, fixed3(0.5, 0, index), bottomColor);
							AppendVertex(triStream, grassPosition, -displacementx + half4(0, size.y / 2, 0, 0) - displacement, upNormal, fixed3(1, 0, index), bottomColor);
						}
					}
				}
				...
				CGPROGRAM
					...
					void geom(point uint p[1] : TEXCOORD, inout TriangleStream<FramentInput> triStream)
					{
						...
						if (GemerateGeometry(p[0], grassPosition, displacement, displacementx, size, tilt, tiltx, bottomColor, topColor, uvDistortion, textureWaving, index))
						{
							...
							{
								// Top vertices
								AppendVertex(triStream, grassPosition, displacement + fixed4(0, size.y, 0, 0), upNormal, fixed3(0 + textureWaving.x, 0.5f, index), topColor);
								AppendVertex(triStream, grassPosition, -displacement + fixed4(0, size.y, 0, 0), upNormal, fixed3(0.5 + textureWaving.y, 0.5f, index), topColor);
							...
							else // ************ Side ************
							{
								...
								// Top vertices (crossed)
								AppendVertex(triStream, grassPosition, displacementx + fixed4(0, size.y, 0, 0), upNormal, fixed3(0 + textureWaving.x, 1, index), topColor);
								AppendVertex(triStream, grassPosition, -displacementx + fixed4(0, size.y, 0, 0), upNormal, fixed3(0.5 + textureWaving.y, 1, index), topColor);
